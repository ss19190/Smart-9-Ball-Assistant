...
\section{Configuration File (config.json)}
The user-specific configuration file utilized to define anthropometric data for the physics engine.

\begin{lstlisting}[language=Java, caption={Content of config.json}, label={lst:config_json}]
{
    "body_weight_kg": 54,
    "gender": "female"
}
...
\section{Vision Module (bilard.py)}
...
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\scriptsize, breaklines=true, caption={Vision Module implementation}, label={lst:bilard_py}]
import cv2
...
\section{Telemetry Module (sensors.py)}
...
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\scriptsize, breaklines=true, caption={Telemetry Module implementation}, label={lst:sensors_py}]
import cv2
import numpy as np
import math
import socket
...
def load_arm_mass():
    ...
    if not os.path.exists(CONFIG_FILE):
        print(f"Config file not found. Using default arm mass: {default_mass} kg")
        return default_mass
    ...
    try:
        ...
        if gender == "female":
            ...
            print(f"Loaded settings for FEMALE: {weight}kg -> Arm mass: {bio_arm_mass:.2f} kg (4.97%)")
        else:
            bio_arm_mass = weight * ARM_PERCENTAGE_MALE
            print(f"Loaded settings for MALE: {weight}kg -> Arm mass: {bio_arm_mass:.2f} kg (5.77%)")
