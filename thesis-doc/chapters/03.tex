...
The initial phase focused on the preparation of the Computer Vision module. To ensure the model's reliability and ability to generalize across different environments three datasets were used. One was a collection of private images captured on both standard club-size table and smaller home table and two were open-source datasets available via Roboflow Universe.

For the \textbf{Ball Detection} model, the custom dataset was augmented with the ``8-pool Dataset" \cite{bib:8-pool_dataset}. Similarly, the \textbf{Cue Detection} model combined images with the ``cue detection Dataset" \cite{bib:cue-detection_dataset}. Both external datasets are licensed under the Creative Commons 4.0 (CC BY 4.0) license.

To strictly follow machine learning best practices, the final aggregated datasets were partitioned using a fixed split ratio:
\begin{itemize}
    \item \textbf{70\% Training Set:} Used for model learning and weight adjustment.
    \item \textbf{20\% Validation Set:} Used for hyperparameter tuning and threshold selection (as described in Chapter 6).
    \item \textbf{10\% Test Set:} Held out completely to evaluate the final performance metrics.
\end{itemize}

As shown in the dataset analysis (Figure~\ref{fig:labels_ball}), the data reflects the natural logic of the game. Specifically, chart (a) highlights the significant class imbalance: the ``other" class (object balls 1-15) is approximately 15 times more frequent than the ``cue ball" class.

Chart (b) visualizes all bounding boxes in the dataset centered at one central point. Since the camera is mounted in a fixed top-down position and all billiard balls have the same physical size, every detected object occupies roughly the same number of pixels. Therefore, all bounding boxes have nearly identical width and height, forming a single, square shape on the plot.

The spatial distribution (c) confirms that balls are clustered within the table area, while the size correlation (d) indicates consistent object dimensions across the dataset.
...
A similar process was followed for the \textbf{Cue Detection model}, where the dataset was annotated with keypoints (skeleton) to define the tip and handle positions for the pose estimation task. The class distribution for this dataset (Figure~\ref{fig:labels_cue}) is uniform as it focuses on a single class, ``cue".
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{./graf/ball-detection/labels-ball.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{./graf/cue-detection/labels-cue.pdf}
