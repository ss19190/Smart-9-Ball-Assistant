...
This chapter details the functional and non-functional requirements of the "smart pool assistant" and presents the technological stack selected for implementation. It also describes the system's use cases and the method adopted for the design and development process.
...
The Android application was developed to reliably extract linear acceleration and gyroscope data. The focus was on implementing the `type\_linear\_acceleration` \cite{bib:linear} sensor to filter out gravity and developing a stable TCP socket protocol for data transmission.